...
int main(int argc, char **argv) {
  ...
  auto in_bohdl0 = xrt::bo(device, 65536 * sizeof(float), top_0.group_id(0));
  ...
  memcpy(in_bomapped0, srcVec0.data(), srcVec0.size() * sizeof(float));  in_bohdl0.sync(XCL_BO_SYNC_BO_TO_DEVICE, 65536 * sizeof(float), 0);
  ...
  auto in_bohdl1 = xrt::bo(device, 65536 * sizeof(float), top_0.group_id(0));
  ...
  memcpy(in_bomapped1, srcVec1.data(), srcVec1.size() * sizeof(float));  in_bohdl1.sync(XCL_BO_SYNC_BO_TO_DEVICE, 65536 * sizeof(float), 0);
  ...
  auto out_bohdl0 = xrt::bo(device, 65536 * sizeof(float), top_0.group_id(0));
  ...
  out_bohdl0.sync(XCL_BO_SYNC_BO_FROM_DEVICE , 65536 * sizeof(float), 0);
  ...
  if(verify){
    ...
    for (unsigned i=0; i < 65536; i++){
      if(abs((float)(srcVec2[i]-out_bomapped0[i])>=1e-4)){
        printf("Error found srcVec2[%d]!=out_bomapped0[%d], %f!=%f ", i, i, srcVec2[i], out_bomapped0[i]);
        errorCount++;
      }
    }
    if (errorCount)
