...
module {
  ...
  func.func @gemm(%A: memref<256x256xf32>, %B: memref<256x256xf32>, %C: memref<256x256xf32>) attributes {adf.func} {
    affine.for %i = 0 to 8 {
      affine.for %j = 0 to 8 {
        affine.for %k = 0 to 8 {
          ...
          adf.dma(%A[%v0 ,%v1] [%cti_size ,%ctk_size] [%cti_stride ,%ctk_stride] [] [] [] [], %L1_A[] [] [] [] [] [] []) : (memref<256x256xf32> , memref<32x32xf32, 2>)
          ...
          adf.dma(%B[%v2 ,%v3] [%ctk_size_1 ,%ctj_size] [%ctk_stride_1 ,%ctj_stride] [] [] [] [], %L1_B[] [] [] [] [] [] []) : (memref<256x256xf32> , memref<32x32xf32, 2>)
          ...
          adf.dma(%L1_C[] [] [] [] [] [] [], %C[%v4 ,%v5] [%cti_size_1 ,%ctj_size_1] [%cti_stride_1 ,%ctj_stride_1] [] [] [] []) {accumulator} : (memref<32x32xf32, 2> , memref<256x256xf32>)
        }
      }
    }
    return
  } 
  func.func @top(%A: memref<256x256xf32>, %B: memref<256x256xf32>, %C: memref<256x256xf32>) attributes {top_func} {
    func.call @gemm(%A, %B, %C) : (memref<256x256xf32>, memref<256x256xf32>, memref<256x256xf32>) -> ()
    return
  } 
}
